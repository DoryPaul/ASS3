...
data_A6 = data.filter(['age', 'sex' ,'fasting blood sugar'], axis = 1)
...
for i in range(2):
    ...
    for j in range(len(bars)):
        male_sub.append(len(data_A6[(data_A6['age'] < (30 + 5 * j)) & (data_A6['age'] >= (25 + 5 * j)) & (data_A6['sex'] == 1) & (data_A6['fasting blood sugar'] == i)]))
        female_sub.append(len(data_A6[(data_A6['age'] < (30 + 5 * j)) & (data_A6['age'] >= (25 + 5 * j)) & (data_A6['sex'] == 0) & (data_A6['fasting blood sugar'] == i)]))
...
for i in range(len(bars)):
    ...
    if female_sub_total != 0:
        ...
        female[0][i] = 0
        female[1][i] = 0
plt.bar(x, male[0], width = bar_width, label = 'male fasting blood sugar', color = 'r', zorder = 2)
plt.bar(x1, female[0], width = bar_width, label = 'female fasting blood sugar', color = 'r', zorder = 2)
...
plt.suptitle('Fasting Blood Sugar', fontsize=16, horizontalalignment = 'center')
plt.xlabel('Age Group')
plt.ylabel('Percentage')
plt.xticks(x2, bars)
red_label = mpatches.Patch(color = 'r', label = 'fasting blood sugar <= 120 mg/dl')
blue_label = mpatches.Patch(color = 'b', label = 'fasting blood sugar > 120 mg/dl')
plt.legend(handles=[red_label, blue_label], loc = 'upper center', bbox_to_anchor=(0.5, -0.2))
plt.grid(axis = 'y')
plt.savefig("A6", bbox_inches = 'tight')
